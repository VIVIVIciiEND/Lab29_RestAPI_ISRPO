...
public class TasksController : ControllerBase {
    private static List<TaskItem> _tasks = new() {
        new TaskItem {
            Id = 1 , Title = "изучить asp.net core" , Priority = "high" , IsCompleted = true
        } ,
new TaskItem {
            Id = 2 , Title = "сделать лабораторную" , Priority = "high" , IsCompleted = false
        } ,
new TaskItem {
            Id = 3 , Title = "написать readme" , Priority = "normal" , IsCompleted = false
        } ,
    };
    ...
    public ActionResult<TaskItem> Create([FromBody] CreateTaskDto dto) {
        if (string.IsNullOrWhiteSpace(dto.Title))
            return BadRequest(new { Message = "поле title обязательно дл заполнения " });
    ...
    public ActionResult Delete(int id) {
        var task = _tasks.FirstOrDefault(t => t.Id == id);
        if (task is null)
            return NotFound(new { Message = $"задача c id = {id} не найдена" });
        _tasks.Remove(task);
        return NoContent();
    }
    ...
    public ActionResult<IEnumerable<TaskItem>> GetByPriority(string level) {
        ...
        if (!allowed.Contains(level, StringComparer.OrdinalIgnoreCase))
            return BadRequest(new { Message = "допустимые значчения low , normal , high" });
        var results = _tasks
